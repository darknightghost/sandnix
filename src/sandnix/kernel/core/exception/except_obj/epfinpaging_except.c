...
static	pkstring_obj_t	to_string(pepfinpaging_except_t p_this);
static	void			panic(pepfinpaging_except_t p_this);

pepfinpaging_except_t epfinpaging_except(address_t fault_addr)
{
    pepfinpaging_except_t p_ret = (pepfinpaging_except_t)except_obj(
                                      sizeof(epfinpaging_except_t),
                                      EPFINPAGING);

    if(p_ret != NULL) {
        p_ret->except.obj.class_id = CLASS_EXCEPT(EPFINPAGING);
...
pkstring_obj_t to_string(pepfinpaging_except_t p_this)
{
    pkstring_obj_t p_ret = kstring_fmt(
                               "%s\n"
                               "Page fault in paging module at %p.",
                               p_except_obj_heap,
...
void panic(pepfinpaging_except_t p_this)
{
    //Get call stack
    list_t bt_lst;
