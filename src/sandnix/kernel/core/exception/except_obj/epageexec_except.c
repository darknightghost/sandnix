...
static	pkstring_obj_t	to_string(pepageexec_except_t p_this);
static	void			panic(pepageexec_except_t p_this);

pepageexec_except_t epageexec_except(address_t fault_addr)
{
    pepageexec_except_t p_ret = (pepageexec_except_t)except_obj(
                                    sizeof(epageexec_except_t),
                                    EDIV);

    if(p_ret != NULL) {
        p_ret->except.obj.class_id = CLASS_EXCEPT(EPAGEEXEC);
...
pkstring_obj_t to_string(pepageexec_except_t p_this)
{
    pkstring_obj_t p_ret = kstring_fmt(
                               "%s\n"
                               "Failed to exec memory at %p.",
                               p_except_obj_heap,
...
void panic(pepageexec_except_t p_this)
{
    //Get call stack
    list_t bt_lst;
