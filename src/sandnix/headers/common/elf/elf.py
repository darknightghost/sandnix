...
class program_header:
    ...
    FLAGS_OFF32 = MEMSZ_OFF32 + 4
    FLAGS_OFF64 = MEMSZ_OFF64 + 8
    def	__init__(self,data,file,header_type,machine):
        ...
        if header_type == elf.ELFCLASS32:
            ...
            self.flags = struct.unpack("<I",data[self.FLAGS_OFF32 : self.FLAGS_OFF32 + 4])[0]
            ...
            self.flags = struct.unpack("<I",data[self.FLAGS_OFF64 : self.FLAGS_OFF64 + 8])[0]
    ...
    def __str__(self):
        return "\nSegment:\n" + "Offset : 0x%0.16X.\n"%(self.off) \
                + "Virtual address : 0x%0.16X.\n"%(self.vaddr) \
                + "Size in file : 0x%0.16X.\n"%(self.filesz) \
                + "Size in memory : 0x%0.16X.\n"%(self.memsz) \
                + "Flags : 0x%.2X.\n"%(self.flags)
...
class elf:
    ...
    def __init__(self,path):
        ...
        self.machine = struct.unpack("<H",self.file.read(2))[0]
    ...
    def __str__(self):
        ...
        if self.type == self.ELFCLASS32:
            ret = ret + "Kernel file type : elf32.\n"
        elif self.type == self.ELFCLASS64:
            ret = ret + "Kernel file type : elf64.\n"

        ret = ret + "Machine : 0x%0.4X.\n"%(self.machine)
        ret = ret + "Entery : 0x%0.8X.\n"%(self.entry)

        for s in self.program_headers:
            ret = ret + str(s)
    ...
    def check_indent(self,path):
        ...
        if struct.unpack("b",bytes([data[0]]))[0] != 0x7F \
            or struct.unpack("b",bytes([data[1]]))[0] != 0x45 \
            or struct.unpack("b",bytes([data[2]]))[0] != 0x4C \
            or struct.unpack("b",bytes([data[3]]))[0] != 0x46:
        ...
        if struct.unpack("b",bytes([data[0]]))[0] in (self.ELFCLASS32,self.ELFCLASS64):
