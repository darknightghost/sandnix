...
bool ext2_open(pfile fp, char* path)
{
	...
	memset(p_file_info, 0, sizeof(ext2_file_info));
	//Compute block size
	...
	while(*p != '\0') {
		...
		if(!hdd_read(fp->disk_info,
		             fp->partition_lba + block * p_file_info->block_size / HDD_SECTOR_SIZE,
		             p_file_info->block_size / HDD_SECTOR_SIZE, (void*)block_buf)) {
			free(p_group_desc);
			free(p_file_info);
			free(p_super_block);
			free(block_buf);
	...
	memcpy(&(p_file_info->inode), p_inode, sizeof(ext2_inode));
...
u32 get_file_inode(pfile fp, pext2_inode p_parent_inode, u32 block_size, bool* is_directory, char* name)
{
	...
	while(read_len < p_parent_inode->i_size) {
		...
		if(p_dir_entry->name_len == 0
		   || p_dir_entry->rec_len == 0) {
			break;
		}

		//Get dir name

		if(p_dir_entry->rec_len > 256) {
			p_dir_entry->rec_len = EXT2_DIR_ENTRY_BASIC_SIZE + p_dir_entry->name_len;
		}
		...
		//Compare dir name
		p_dir_entry->name[p_dir_entry->name_len] = '\0';
