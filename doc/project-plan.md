...
##### 缩进
缩进为4个空格,8个空格会使代码很靠右,尽量用空格而不是tab(这一点利用astyle可以很轻松的搞定)..因为github上面tab看起来相当的糟糕.switch语句里面的case应当缩进来使得代码层次清晰.
不要一行放多个语句.
...
```
#####io
负责中断的管理,虚拟IRQ的映射,系统时钟的管理,以及提供必要的寄存器接口供hal层内部使用.
######模块路径
```
