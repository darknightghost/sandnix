...
class listcontrol(control):
    ...
    def draw(self,pos,begin,max):
        ...
        self.wnd.addstr(self.pos.top,
            self.pos.left, "%s : [%s]"%(self.data[1],self.selections[0][self.selections[1]]), self.color)
        return 1
    ...
    def refresh(self):
        ...
        self.wnd.addstr(self.pos.top,
            self.pos.left, "%s : [%s]"%(self.data[1],self.selections[0][self.selections[1]]), color)
        self.wnd.refresh()
        return
    ...
    def draw_wnd(self):
        ...
        title = self.data[1]
        self.client_wnd.addstr(0, int(self.client_rect.width / 2 - len(title) / 2),
            ...
            if i + self.begin == self.selections[1]:
                if i + self.begin == self.focused_item:
                    self.selection_wnd.addstr(1 + i,1,
                        self.selections[0][i + self.begin], self.color_selected | curses.A_REVERSE)
                else:
                    self.selection_wnd.addstr(1 + i,1,
                        self.selections[0][i + self.begin], self.color_selected)
            else:
                if i + self.begin == self.focused_item:
                    self.selection_wnd.addstr(1 + i,1,
                        self.selections[0][i + self.begin], self.color_selection | curses.A_REVERSE)
                else:
                    self.selection_wnd.addstr(1 + i,1,
                        self.selections[0][i + self.begin], self.color_selection)
            i = i + 1
